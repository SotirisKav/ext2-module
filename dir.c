 ...
 */
 ...
 static void ext2_commit_chunk(struct folio *folio, loff_t pos, unsigned len)
 {
	 ...
	 block_write_end(pos, len, len, folio); // block_write_end(NULL, mapping, pos, len, len, &folio->page, NULL);
 ...
 static int ext2_readdir(struct file *file, struct dir_context *ctx)
 {
	 ...
	 bool need_revalidate = true; // bool need_revalidate = !inode_eq_iversion(inode, file->f_version);
	 ...
	 for ( ; n < npages; n++, offset = 0) {
		 ...
		 for ( ; (char*)de <= limit; de = ext2_next_entry(de)) {
			 if (de->rec_len == 0) {
				 ...
				 folio_release_kmap(folio, kaddr); //changed to kaddr
				 return -EIO;
			 }
			 if (de->inode) {
															...
															struct folio **foliop)
 {
	 ...
	 for (i=0; i < npages; i++) {
		...
		limit = kaddr + ext2_last_byte(dir, i) - reclen;
