...
static int ext2_write_begin(const struct kiocb *iocb, struct address_space *mapping,
                            loff_t pos, unsigned len, 
...
static int ext2_write_end(const struct kiocb *iocb, struct address_space *mapping,
                          loff_t pos, unsigned len, unsigned copied,
                                         ...
                                         struct buffer_head **p)
{
	...
	*p = NULL;
	...
	/* ? */
	ino--; //because inodes are 1 indexed
	block_group=ino/inodes_pg;
	gdp=ext2_get_group_desc(sb, block_group, NULL); //get block group descriptor
	if (gdp==NULL){
		goto eio;
	}
	/* Figure out the offset within the block group inode table */
	/* ? */
	unsigned long inodes_per_block = blocksize / inode_sz;
	unsigned long inode_index=ino%inodes_pg;
	block=le32_to_cpu(gdp->bg_inode_table)+(inode_index)/inodes_per_block;
	offset=(inode_index%inodes_per_block)*inode_sz;
